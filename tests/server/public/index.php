...
$app->router->get('/string-response', function () {
...
$app->router->get('/basic-auth', function() {
    $headers = [
        (bool) preg_match('/Basic\s[a-zA-Z0-9]+/', app('request')->header('Authorization')),
            app('request')->header('php-auth-user') === 'net-call',
        app('request')->header('php-auth-pw') === 'secret'
    ];

    return (count(array_unique($headers)) === 1) ?
        response(null, 200) :
        response(null, 401);
});
...
$app->router->get('/digest-auth', function() {
    ...
    if ($data['username'] != 'net-call') {
        return response(null, 401);
    }

    $a = md5('net-call:' . $realm . ':secret');
    $b = md5(app('request')->server->get('REQUEST_METHOD') . ':' . $data['uri']);
    $validResponse = md5(
        $a . ':' . $data['nonce'] . ':' . $data['nc'] . ':'.$data['cnonce'] . ':' . $data['qop'] . ':' . $b
    );
